...
## No Flutter MCP server is connected in this environment

There's no dedicated Flutter/Dart MCP server configured for this project — Flutter/Dart tooling (`flutter analyze`, `flutter test`, `flutter pub`, `dart run build_runner`) runs through Bash. The `android-cli` skill (if invoked) covers the `android` CLI for SDK/AVD/device management, which is a separate concern from Flutter itself. Style/architecture guidance instead comes from the `flutter-project-rules` skill plus this file — treat those as the "Flutter skill" for this repo rather than expecting a live MCP tool.
